...
2. `E` a real thread → it's archived in Gmail (check the web UI). Reply from Gmail → the thread returns to the Snail Mail inbox on the next sync (≤60s).
...
1. Windows *Credential Manager* → Windows Credentials → entries under `SnailMail` (AI keys, OAuth) — that's where everything lives. Installs that predate the renames may also show `FissionMail`/`ZenBoxMail` copies; reads migrate forward lazily.

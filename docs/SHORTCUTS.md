...
Replying: `R`/`A`/`F` (or `Enter` in a thread) thread the composer **inline into the conversation**, at the same width as the email above it. Recipients collapse to a one-line summary (`Ctrl+Shift+O/C/B/S` to edit a field). Select text in your message to get a floating **formatting bubble** (bold, italic, underline, link, text color, bulleted/numbered list, block quote). Your **signature and the quoted email** are tucked behind a subtle `•••` and **render faithfully** (real email HTML — tables, images, layout) — press `↓` to focus the `•••` then `Enter`, or click it. They're appended to the message on send. New-message compose (`C`) opens as a centered window with a persistent toolbar.

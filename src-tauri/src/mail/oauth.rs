    ...
    let code = tokio::time::timeout(std::time::Duration::from_secs(300), async {
        loop {
            ...
            let (status, body) = if error.is_some() {
                ("200 OK", "<h2>Snail Mail</h2><p>Access was denied. You can close this tab and try again from the app.</p>")
            } else if code.is_some() && got_state.as_deref() == Some(state.as_str()) {
                ("200 OK", "<h2>Snail Mail</h2><p>Connected. You can close this tab and return to the app.</p>")
            } else {
